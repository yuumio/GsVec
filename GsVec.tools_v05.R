...
gs.train_tfidf <- function(
	train.data = train.data
){

	ggm <- itoken(as.character(as.matrix(train.data$gene)), preprocess_function = toupper, tokenizer = space_tokenizer, ids = train.data$id)
	genes <- create_vocabulary(ggm)
	genes <- genes[order(genes$term),]
	vectorizer <- vocab_vectorizer(genes)
	ggm <- create_dtm(ggm, vectorizer)

	# TF-IDF
	tfidf <- TfIdf$new(smooth_idf = TRUE, norm = "l1", sublinear_tf = FALSE)
	train_tfidf <- t(as.matrix(tfidf$fit_transform(ggm)))
...
gs.train_glove <- function(
	...
	# make gene co occuerence matrix
	gcm <- itoken(as.character(as.matrix(train.data$gene)), preprocess_function = toupper, tokenizer = space_tokenizer, ids = train.data$id)
	genes <- create_vocabulary(gcm)
	genes <- genes[order(genes$term),]
	vectorizer <- vocab_vectorizer(genes)
	gcm <- create_tcm(gcm, vectorizer, skip_grams_window = 5)

	#
	glove_model <-  GlobalVectors$new(word_vectors_size = train_vectorsize, vocabulary = genes, lambda = lambda, x_max=x_max,alpha=alpha,
			learning_rate = 0.15)
	fit <- glove_model$fit_transform(gcm ,n_iter = train_epochs, convergence_tol = -1, n_threads = 20)
	gene_vectors_context = t(glove_model$components)
#	train_glove = normalize(t(fit) + gene_vectors_context,norm="l1")
	train_glove = fit + gene_vectors_context
...
## -------------------------------------------------------------------------------------------------- export_topic.clusters

export_topic.clusters <- function(
	topic.vec = train.fm,
	pred.gsvec = pred.data,
	cluster_data = train.cluster,
	cluster_method = c("gmm","lda"),
	export_clutser.p = T,
	export_top.genes =T,
	top.genes_n = 10,
	assign.clusters_gene = T,
	plot.tsne_gene = T,
	assign.clutsers_gs = T,
	plot.tsne_gs = T,
	import_gs.tsne_txt.data = "tSNE.data__.txt", # or NA, ""
	compres_pca = T,
	pca_thres = 0.95,#NA
	pca_centering = F,
	tsne_max.iter = 500,
	...
	if(tolower(substr(cluster_method,1,1)) == "g"){
		cluster.p <- cluster_data$z
		colnames(cluster.p) <- 1:ncol(cluster.p)
	}
	if(tolower(substr(cluster_method,1,1)) == "l"){
		cluster.p <- cluster_data$theta
	}
	if(export_clutser.p == T){
		out <- cbind(rownames(cluster.p),cluster.p)
		fwrite(as.data.table(out),paste0("cluster.prob__",feature.name,".txt"),sep="\t",quote=F)
		if(verbose ==T){cat(paste0("\n - exported cluster probabilities"))}
	}

	if(export_top.genes == T){
		cluster_top.genes <- function(p,g){
			x <- c(g[order(p,decreasing = T)])[1:top.genes_n]
			x[c(p[order(p,decreasing = T)])[1:top.genes_n] == 0] <- ""
			return(x)
		}
		out <- apply(cluster.p,2,function(p) cluster_top.genes(p,rownames(cluster.p)))

		colnames(out) <- paste0("cluster_",1:ncol(cluster.p))
		fwrite(as.data.table(out),paste0("cluster.top",top.genes_n,"__",feature.name,".txt"),sep="\t",quote=F)	
		if(verbose ==T){cat(paste0("\n - exported cluster top genes"))}
	}

	if(tolower(substr(cluster_method,1,1)) == "g"){
		cls_gene <- data.frame(id = as.character(names(cluster_data$classification)),cluster = as.numeric(as.matrix(cluster_data$classification)), p=0.0)
	}
	if(tolower(substr(cluster_method,1,1)) == "l"){
		cls_gene <- data.frame(id = as.character(rownames(cluster.p)),cluster = apply(cluster.p,1,function(x) colnames(cluster.p)[x ==max(x)]), p=0.0)
	}
	for(i in 1:nrow(cls_gene)){
		cls_gene$p[i] <- cluster.p[i,cls_gene$cluster[i]]
	}

	if(assign.clusters_gene == T){
		fwrite(as.data.table(cls_gene),paste0("cluster.all.genes__",feature.name,".txt"),sep="\t",quote=F,row.names=F)		
		if(verbose ==T){cat(paste0("\n - assigned genes to clutsers"))}
	}

	if(plot.tsne_gene == T){
		if(compres_pca == T){
			pcadata <- prcomp(topic.vec,center = pca_centering)
			indx <- !(summary(pcadata)$importance[3,] > pca_thres)
			pcs <- pcadata$x[,indx]
		}else{
			pcs <- topic.vec
		}
		tsnedata <- Rtsne(pcs, dims = 2, initial_dims = ncol(pcs), perplexity = 30, theta = 0.5, check_duplicates = F, pca = F, 
			max_iter = tsne_max.iter, verbose = T, is_distance = FALSE, Y_init = NULL, eta = tsne_sta, exaggeration_factor = 12)
		data <- data.frame(id=rownames(pcs), TSNE_1 = tsnedata$Y[,1], TSNE_2 = tsnedata$Y[,2], stringsAsFactors=FALSE)
		data <- merge(data,cls_gene,by="id")
		data$cluster <- as.factor(as.numeric(as.matrix(data$cluster)))
		fwrite(data,paste0("tSNE.data_topic.vec_gene__",feature.name,".txt"),sep="\t",quote=F,row.names=F)

		g <- ggplot(data, aes(x = TSNE_1, y = TSNE_2, color = cluster,size=p))
		g <- g + geom_text(aes(x=TSNE_1, y=TSNE_2, label=cluster),alpha=0.3)
		g <- g + labs(title=feature.name)
		ggsave(plot=g,file = paste0("tSNE_topic.vec_gene__",feature.name,".png"), device="png", units="in", dpi = 150, width = 13, height = 10)
		if(verbose ==T){cat(paste0("\n - exported tSNE by genes on topic vectors"))}
	}

	if(assign.clutsers_gs == T | plot.tsne_gs == T){
		centroid.mat <- matrix(0.0,ncol=ncol(topic.vec),nrow=ncol(cluster.p))
		rownames(centroid.mat) <- 1:ncol(cluster.p)
		for(i in 1:nrow(centroid.mat)){
			tmp <- topic.vec[cls_gene$cluster == i,]
			if(is.vector(tmp) == T){
				centroid.mat[i,] <- tmp
			}else{
				centroid.mat[i,] <- apply(as.matrix(tmp),2,mean)
			}
		}
		centroid.sim <- similarity_vectors(
			train_feature.matrix = centroid.mat,
			val_feature.matrix = pred.gsvec,
			feature.name = "centroid.cosine",
			convert.by.melt = F
		)
		centroid.sim_max <- apply(centroid.sim,1,max)
		cls_gs <- data.frame(id = rownames(centroid.sim),cluster="",cosine=0.0, stringsAsFactors=FALSE)
		for(i in 1:nrow(cls_gs)){
			indx <- centroid.sim[i,] == centroid.sim_max[i]
			cls_gs$cluster[i] <- colnames(centroid.sim)[indx]
			cls_gs$cosine[i] <- centroid.sim[i,indx]
		}
		cls_gs$cluster <- as.factor(as.numeric(cls_gs$cluster))

		result <- list()
	}
	if(assign.clutsers_gs == T){
		fwrite(as.data.table(cls_gs),paste0("cluster.all.gs__",feature.name,".txt"),sep="\t",quote=F,row.names=F)		
		if(verbose == T){cat(paste0("\n - assigned gene signatures to clutsers"))}
	}

	if(plot.tsne_gs == T){
		if(is.na(import_gs.tsne_txt.data) | import_gs.tsne_txt.data==""){
			if(compres_pca == T){
				pcadata <- prcomp(pred.gsvec,center = pca_centering)
				indx <- !(summary(pcadata)$importance[3,] > pca_thres)
				pcs <- pcadata$x[,indx]
			}else{
				pcs <- pred.gsvec
			...
			tmp <- as.data.frame(fread(import_gs.tsne_txt.data,stringsAsFactors = F))
			data <- tmp[,c("id","TSNE_1","TSNE_2")]
		}
		data <- merge(data,cls_gs,by="id")
		fwrite(data,paste0("tSNE.data_topic.vec_gs__",feature.name,".txt"),sep="\t",quote=F,row.names=F)

		g <- ggplot(data, aes(x = TSNE_1, y = TSNE_2, color = cluster,size=cosine))
		g <- g + geom_text(aes(x=TSNE_1, y=TSNE_2, label=cluster),alpha=0.3)
		g <- g + labs(title=feature.name)
		ggsave(plot=g,file = paste0("tSNE_topic.vec_gs__",feature.name,".png"), device="png", units="in", dpi = 150, width = 13, height = 10)
		if(verbose ==T){cat(paste0("\n - exported tSNE by gene signatures on topic vectors"))}
	}
	if(exists("result")){
		result[[1]] <- cls_gene
		result[[2]] <- cls_gs
	...
	if(verbose ==T){cat(paste0("\n -------- end of function: export_topic.clusters\n"))}
	return(result)
}
